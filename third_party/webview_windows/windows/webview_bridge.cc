...
void WebviewBridge::HandleMethodCall(
  ...
  // setPreferredColorScheme: int (SALU addition, VENDOR_NOTES.md)
  // 0 = auto (follow the OS), 1 = light, 2 = dark — the
  // CoreWebView2PreferredColorScheme values exactly.
  if (method_name.compare(kMethodSetPreferredColorScheme) == 0) {
    if (const auto scheme = std::get_if<int32_t>(method_call.arguments())) {
      if (*scheme < 0 || *scheme > 2) {
        return result->Error(kErrorInvalidArgs);
      }
      if (webview_->SetPreferredColorScheme(*scheme)) {
        return result->Success();
      }
      return result->Error(kErrorNotSupported,
