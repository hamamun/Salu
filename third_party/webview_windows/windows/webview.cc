...
bool Webview::SetPreferredColorScheme(int scheme) {
  // SALU addition (VENDOR_NOTES.md): the engine's own PreferredColorScheme —
  // the control Microsoft exposes on the profile every view of this browser
  // instance shares, and the same one Edge's Appearance setting drives. It
  // decides what `prefers-color-scheme` answers and applies to the live
  // page immediately; no reload, no restart. Requires runtime/SDK
  // >= 1.0.1155.38 (the SDK pinned above is 1.0.1210.39); on an ancient
  // runtime this is a harmless no-op and the view keeps following the OS.
  if (!webview_) {
    return false;
  }
