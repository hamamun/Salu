...
class WebviewController extends ValueNotifier<WebviewValue> {
  ...
  /// Sets the preferred colour scheme of the profile all views of this
  /// browser instance share (SALU addition, VENDOR_NOTES.md) — the engine's
  /// own control for what `prefers-color-scheme` answers, the same one
  /// Microsoft Edge's Appearance setting drives. [scheme] uses the
  /// CoreWebView2PreferredColorScheme values: 0 = auto (follow the OS),
  /// 1 = light, 2 = dark. Applies to the live page immediately — no reload,
  /// no restart. Fails (throws) on runtimes older than 1.0.1155.38, where
  /// the view simply keeps following the OS.
  Future<void> setPreferredColorScheme(int scheme) async {
    if (_isDisposed) {
      return;
    }
    assert(value.isInitialized);
