...
import 'dart:ui' as ui show Display, PlatformDispatcher, Size;
...
class MpvTuneEngine implements TuneEngine {
  ...
  @override
  Future<void> setSpeed(double speed, {required bool keepPitch}) async {
    final double v = speed.isFinite ? clampRange(speed, 0.0, 100) : 1.0;
  ...
  Future<double?> readFileAspect() async {
    double? out = _asDouble(await _read('video-out-params/aspect'));
    if (out == null || out <= 0.05) {
      // Some builds expose no `aspect` sub-property: derive it from the
      // frame size and the pixel aspect.
      final double? dw = _asDouble(await _read('video-out-params/w'));
      final double? dh = _asDouble(await _read('video-out-params/h'));
      if (dw != null && dh != null && dw > 0 && dh > 0) {
        final double par = _asDouble(await _read('video-out-params/pix-aspect')) ?? 1;
        out = dw * (par > 0 && par.isFinite ? par : 1) / dh;
      }
    }
    if (out == null || !out.isFinite || out <= 0.05) return null;
    return out;
  }
  ...
  static ui.Size _screenSize() {
    final List<ui.Display> displays = ui.PlatformDispatcher.instance.displays;
    if (displays.isEmpty) return ui.Size.zero;
    final ui.Display display = displays.first;
    final double dpr =
        display.devicePixelRatio > 0 ? display.devicePixelRatio : 1.0;
    return ui.Size(
      display.physicalSize.width / dpr,
      display.physicalSize.height / dpr,
    );
  }
}
