...
- [ ] **Phase 4: Slide-Out Panels & Menus** (Right panel for Quick Settings [Video/Audio/Subtitles], slide-out Playlist/Chapter menus). **Implemented:** Player-only right-button strip (Shuffle · Repeat · Info · Settings), left Info panel with defensive metadata collection/live clocks, and one-popup coordination including Open. Remote stays with Phase 8; native Windows verification pending (`info.md` §10).
