...
- **Mode + window** — Player · Web switch top-left of the strip in both
  ...
  downloads ASK where to land (the native Windows Save As, on by
  default — Settings → Web → Downloads, 2026-09-19 lock below) and are
...
- **Memory** — a mode switch keeps the browser alive (2026-09-17, Mode
  ...
  runs only when the app CLOSES (key function 8, the close guard). Both
  of the browser's settings doors — the ⋮ menu's "Settings" row and the
  Web-mode title strip's button — open SALU's Settings window straight
  onto its **Web** tab (`SettingsDialog.initialTab`, 2026-09-19 lock
  below); Player mode's own doors still open General.
...
delete + clear-all · downloads badge + shelf (start → ring → land → Play / Show in folder / Open folder · ⋮ door · title-bar badge in BOTH modes) ·
downloads Save As (ask ON: the dialog pre-filled with the engine's name, starting in the download folder · the chosen path is where it lands · Cancel writes nothing and leaves no shelf row · ask OFF: straight to the folder, no dialog · Download location Change… + reset to Windows · two downloads at once = one dialog at a time · flipping the switch mid-session reaches live tabs) · clear dialog · Edge door ·
Settings opens on the **Web** tab — from the ⋮ row AND the Web-mode title
strip, while Player mode's title-bar button still opens General) · Ctrl+T/W/R/L/F + zoom keys · permission cards · resume memory
...
| 2026-09-16 | **Downloads — LOCKED** | Standard browser behaviour: send the file to **Windows File Explorer** (user's Downloads folder) — like Edge/Chrome. **Superseded 2026-09-19** by *"Downloads: ask where to save + download folder"* below: the standard browser behaviour turns out to include **asking** (Edge and Chrome both ship that switch), and the Downloads folder stays the default answer either way. |
...
| 2026-09-19 | **Downloads: ask where to save + download folder — LOCKED** | Owner's ask: SALU dropped files into Windows Downloads without a word, while Edge/Chrome ask. Cause was never SALU's Dart — the vendored plugin's `DownloadStarting` handler set `Handled`, read the engine's suggested result path and **wrote it straight back**, so the path was settled before Dart heard anything (it only ever got the *report*, never the *decision*). Fix rides the engine's own hook, all of it inside the SDK the plugin already pins (`1.0.1210.39`, so no build change): `put_ResultFilePath` / `put_Cancel` held on a `GetDeferral()`, plus `put_DefaultDownloadFolderPath` on the profile every tab shares. **Settings → Web → Downloads** carries the two halves — **"Ask where to save each file"** (**ON by default**) and **"Download location"** (empty = the Windows Downloads folder, a relocated one included; a path only ever arrives from the native folder picker, with a reset back to Windows). Asking opens the **native Windows Save As** — the same shell dialog Edge and Chrome use, reached through `file_selector` (already a dependency for Open File… / Open Folder…), which is why the overwrite warning and the "file (1).mp4" naming come for free — pre-filled with the engine's own file name, starting in the chosen folder, filtered to the file's own extension. **Cancel is a refusal, not a failure:** the download is cancelled before a byte is written, so it never reaches the log, the shelf or the badge. **One dialog at a time** — a page firing three downloads queues its prompts, each sitting on its own engine deferral until its turn. Both halves apply live to every running tab (`WebDataControlService.applyDownloadPreferences` — the Page-colours contract) and are read at tab birth (`WebTab._start`); a download already travelling keeps the path it was given. Every way the question can fail — asking flipped off while a prompt waits in the queue, no picker, Dart throwing, a runtime too old for the profile API — answers **"your own path is fine"**, never a lost file; a view torn down mid-prompt checks a shared life flag rather than writing into freed memory. **Second vendor delta** (`third_party/webview_windows`, VENDOR_NOTES.md · Delta 2): the plugin gains `setDownloadPreferences` and a `downloadStarting` ask-and-wait leg on the channel modelled on the `permissionRequested` one it already had, and stays UI-free — the dialog is Dart's (`WebDownloadService.askWhereToSave`). The shelf's footer now reads **Open download folder** and follows the setting; **Show in folder** needed no change (it already opens the file's own folder, wherever that now is). Still parked: cancelling or pausing a download that is already travelling — this delta gives SALU a say *before* the first byte only. |
| 2026-09-19 | **Settings doors open on the tab you came from — LOCKED** | Owner's ask: the ⋮ menu's "Settings" row opened SALU's Settings window on **General**, so reaching the browser's own settings from the browser meant hunting across the tab strip for them. Now the window takes an opening tab (`SettingsDialog.initialTab`, default General) and **both** of Web mode's doors ask for **Web** — the ⋮ menu's row and the Web-mode title strip's settings button (they are the same door twice: one window, two handles, `home_screen`'s `_openWebSettings`). Player mode's title-bar button is untouched and still opens General, so nothing outside the browser moved. The tab strip itself is unchanged — the opening tab is only where the window lands; the viewer taps away freely, and a rebuild never throws them back to it. No new setting, no persistence: this is a door's destination, not a preference. |
| 2026-09-19 | **Page colours hardening + engine line — LOCKED** | Owner's report: on their PC, forcing Light still left pixabay.com dark — the push was refused somewhere between the picker and the profile, and every silent catch swallowed it. Two answers. **(1) Warm-up order:** `prepare()` now registers the Page-colours/Downloads listeners FIRST and guards each leg (purge, auto-clear, environment) on its own, so one misbehaving leg can never again cancel live updates; the purge-before-environment order is unchanged. **(2) Silent failures are visible:** every colour push (live + tab birth) reports its outcome; while one stands refused, Settings → Web → Page colours carries a warning (update the WebView2 Runtime via Windows Update, and make sure SALU itself is up to date), and the tab's new footer line names the engine (`Engine · WebView2 <version>`, via the plugin's upstream `getWebViewVersion` — no new vendor delta). Light/Dark/Follow behaviour itself is unchanged. |
...
- `lib/ui/screens/` now contains `browser_screen.dart`; `lib/ui/widgets/`
  ...
  Explorer's two answers + the Save As prompt queue and the two
  shell-dialog seams behind Settings → Web → Downloads) and
  `web_find.dart` (the highlighter script + answer parsing).
